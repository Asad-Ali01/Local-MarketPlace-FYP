...
import { adminAllUsersDetailsService, adminDeleteUserService, adminEditUserService, adminGetStatsService, adminLoginService, adminUserDetailService } from "../services/admin.services";
...
const adminAllUsers = asyncHandler(async(req,res) => {
  const {page,limit,filterStatus,searchUser} = req.query;
    if(typeof filterStatus !== "string" || typeof searchUser !== "string"){
    throw new ApiError(400,"Filter value or search value is not string");
  }
  const {users,totalUsers} = await adminAllUsersDetailsService(Number(page),Number(limit),filterStatus,searchUser)
